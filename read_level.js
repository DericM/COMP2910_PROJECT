...
function read_level(grid, level_num) {
  var new_array = [];
  ...
  for (var i = 0; i < level[level_num].length; i++) {
    ...
    if (ch == 'x') {
      ...
      new_array[row][column] = new WhiteHouse(row, column, RESOURSE.whitehouse);

    } else if (ch == 't') {
      new_array[row][column] = new Trump(grid, row, column, RESOURCE.trump);

    } else if (ch == '*') {
      new_array[row][column] = new Fadable(row, column, RESOURCE.mine);
